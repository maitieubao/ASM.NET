...
public class DashboardService : IDashboardService
{
    ...
    public async Task<DashboardDto> GetStatsAsync(CancellationToken ct = default)
    {
        ...
        // FIX: Dùng ListeningHistory để đếm lượt nghe thực tế trong app
        // Song.PlayCount bị inflate từ YouTube ViewCount/10000 khi import — không phản ánh thực tế
        var topSongs = await repoHistory
            .GroupBy(h => h.SongId)
            .Select(g => new
            {
                SongId = g.Key,
                RealPlayCount = (long)g.Count()
            })
            .OrderByDescending(x => x.RealPlayCount)
            .Take(10)
            .Join(
                _unitOfWork.Repository<Song>().Query().AsNoTracking()
                    .Where(s => !s.IsDeleted)
                ...
                (h, s) => new TopSongDto
                {
                    ...
                    PlayCount = h.RealPlayCount
                }
            )
            .ToListAsync(ct);

        // FIX: Dùng ListeningHistory để đếm lượt nghe thực tế theo nghệ sĩ
        var topArtists = await repoHistory
            .Join(
                _unitOfWork.Repository<SongArtist>().Query().AsNoTracking()
                    .Include(sa => sa.Artist),
                h => h.SongId,
                sa => sa.SongId,
                (h, sa) => new { sa.ArtistId, sa.Artist.Name }
            )
            .GroupBy(x => new { x.ArtistId, x.Name })
            .Select(g => new TopArtistDto
            {
                Name = g.Key.Name,
                SongCount = g.Select(x => x.ArtistId).Distinct().Count(),
                TotalPlays = (long)g.Count()
            })
