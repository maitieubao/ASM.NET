    ...
    public async Task<bool> GrantPremiumByPlanAsync(int userId, int planId, CancellationToken ct = default)
    {
        try
        {
            ...
            var plan = await _unitOfWork.Repository<SubscriptionPlan>()
                .FirstOrDefaultAsync(p => p.PlanId == planId && p.IsActive, ct);
            if (plan == null) return false;

            var now = DateTime.UtcNow;
            var endDate = plan.DurationDays >= 99999
            ...
            if (activeSubscription == null)
            {
                ...
                await _unitOfWork.Repository<UserSubscription>().AddAsync(newSubscription, ct);
            }
            else
            {
                ...
                _unitOfWork.Repository<UserSubscription>().Update(activeSubscription);
            }
    ...
    public async Task<bool> RevokePremiumAsync(int userId, CancellationToken ct = default)
    {
        try
        {
            ...
            var activeSubscriptions = await _unitOfWork.Repository<UserSubscription>()
                .FindAsync(s => s.UserId == userId && s.IsActive, ct);
