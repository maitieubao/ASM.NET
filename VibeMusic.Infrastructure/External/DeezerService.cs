...
public class DeezerService : IDeezerService
{
    ...
    public async Task<IEnumerable<DeezerAlbumInfo>> SearchAlbumsAsync(string query, int limit = 10)
    {
        ...
        try
        {
            ...
            // FIX PERF: Tăng TTL từ 1 giờ → 6 giờ (album data thay đổi chậm)
            _cache.Set(cacheKey, results, TimeSpan.FromHours(6));
            return results;
        }
        catch { return Enumerable.Empty<DeezerAlbumInfo>(); }
    ...
    {
        // FIX PERF: Tăng TTL từ 30 phút → 4 giờ (new releases không thay đổi thường xuyên)
        ...
        try
        {
            // FIX PERF: Chạy song song editorial + chart thay vì tuần tự
            // Giảm số artist từ 6 → 3 để giảm Deezer API calls
            var editorialTask = GetJsonAsync($"{BaseUrl}/editorial/0/releases?limit=20");
            var chartTask = GetPlaylistTracksAsync(VietnamChartId.ToString(), 20);

            await Task.WhenAll(editorialTask, chartTask);

            var editorial = await editorialTask;
            ...
            {
                var chartTracks = await chartTask;
            ...
            if (final.Any()) {
                _cache.Set(cacheKey, (IEnumerable<DeezerAlbumInfo>)final, TimeSpan.FromHours(4));
                return final;
            }
        } catch { }
