...
public class PlaylistController : BaseController
{
    ...
    {
        if (CurrentUserId == null) return Unauthorized();
        if (string.IsNullOrEmpty(youtubeId)) return BadRequestResponse("YouTube ID không hợp lệ.");

        try
        {
            // Thử lấy/tạo bài hát với retry — ImportAndReturnSongAsync có thể fail lần đầu do Npgsql transient error
            SongDto? song = null;
            ...
            for (int attempt = 1; attempt <= maxAttempts; attempt++)
            {
                ...
                if (attempt < maxAttempts)
                    await Task.Delay(TimeSpan.FromSeconds(attempt)); // 1s, 2s backoff
            }
            ...
            return BadRequestResponse("Không thể xử lý bài hát từ YouTube. Vui lòng thử lại sau.");
        }
        catch (UnauthorizedAccessException)
        {
            return Forbid();
