...
public class AdminArtistController : BaseController
{
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> RefreshArtistBio(int id, CancellationToken ct = default)
    {
        var bio = await _artistService.RefreshArtistBioAsync(id, ct);
        TempData[bio == null ? "Error" : "Success"] = bio == null
            ? "Could not refresh the artist biography from Wikipedia."
            : "Artist biography refreshed successfully.";
