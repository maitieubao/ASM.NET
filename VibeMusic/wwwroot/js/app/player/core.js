...
$(function() {
    ...
    // Manual pause state flag — true khi user chủ động pause, false khi user chủ động play
    // Dùng để phân biệt "manual pause" vs "browser throttle pause" trong heartbeat
    ...
    $playPauseBtn.on('click', async () => {
        ...
        if (audioPlayer.paused) {
            window.isManuallyPaused = false; // User chủ động play — reset manual pause flag
            ...
            window.isManuallyPaused = true; // User chủ động pause — set manual pause flag
            audioPlayer.pause();
            if (typeof updatePlayPauseUI === 'function') updatePlayPauseUI(false);
    ...
    setInterval(() => {
        if (window.audioPlayer && window.playQueue[window.currentIndex]) {
            ...
            const isManualPause = window.isManuallyPaused; // FIX: dùng flag thay vì DOM class check sai
