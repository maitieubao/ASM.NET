...
window.queueSourceType = 'general'; // 'general', 'album', 'playlist'
...
window.saveQueueState = function() {
    const state = {
        ...
        queueSourceType: window.queueSourceType,
        timestamp: Date.now()
    };
...
window.playPlaylist = async function(data, shuffled = false, defaultThumb = null) {
    ...
    window.queueSourceType = 'album'; // Set source as album/playlist to prevent discovery refill
    let tracks = [];
...
window.checkAndRefillQueue = function() {
    if (window.queueSourceType !== 'general') {
        console.log("[Queue] Discovery refill skipped: Queue is in", window.queueSourceType, "mode.");
        return;
    }
