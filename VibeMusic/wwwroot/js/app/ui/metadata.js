    ...
    $.getJSON(url)
        .done(function(json) {
            ...
            if (!data || (data.status === "NOT_FOUND" && retryCount >= 1)) {
                // Bài hát chỉ có trên YouTube, không có metadata/lyrics — hiển thị graceful message
                if ($lc.length) {
                    $lc.removeClass('lyrics-text is-synchronized').html(`
                ...
                if ($bc.length) $bc.text("Thông tin nghệ sĩ đang được cập nhật...");
                return;
            }
        ...
        .fail(function() {
            // Network error hoặc server error — hiển thị graceful message
            if ($lc.length) {
                $lc.removeClass('lyrics-text is-synchronized').html(`
                    <div class="lyrics-not-found animate-fade-in">
                        <i class="fa-solid fa-music mb-3 opacity-20 d-block fs-1"></i>
